const baseSettings = [
  {
    setting: {
      number_of_image_per_product: "5",
      shop_name: "KachaBazar",
      address: "59 Station Rd, Purls Bridge, United Kingdom",
      company_name: "HtmlLover ltd",
      vat_number: "47589",
      post_code: "2030",
      contact: "019579034",
      email: "kachabazar@gmail.com",
      website: "kachabazar-admin.vercel.app",
      default_currency: "$",
      default_time_zone: "Europe/London",
      default_date_format: "MMM D, YYYY",
      receipt_size: "57-mm",
      from_email: "sale@kachabazar.com",
      email_to_customer: false,
      allow_auto_trans: false,
      translation_key: "",
    },
    name: "globalSetting",
  },
  {
    setting: {
      navbar: {
        categories_menu_status: true,
        about_menu_status: true,
        contact_menu_status: true,
        offers_menu_status: true,
        term_and_condition_status: true,
        privacy_policy_status: true,
        faq_status: true,
        help_text: {
          en: "We are available 24/7, Need help?",
          es: "Nosotros estamos disponibles 24/7, Necesitas ayuda?",
        },
        categories: {
          en: "Categories",
          es: "Categorias",
        },
        about_us: {
          en: "About Us",
          es: "Sobre Nosotros",
        },
        contact_us: {
          en: "Contact Us",
          es: "Contactanos",
        },
        offers: {
          en: "Offers",
          es: "Ofertas",
        },
        faq: {
          en: "FAQ",
          es: "FAQs",
        },
        privacy_policy: {
          en: "Privacy Policy",
          es: "Política de privacidad",
        },
        term_and_condition: {
          en: "Terms & Conditions",
          es: "Términos y Condiciones",
        },
        pages: {
          en: "Pages",
          es: "Páginas",
        },
        my_account: {
          en: "My Account",
          es: "Mi cuenta",
        },
        login: {
          en: "Login",
          es: "Iniciar sesión",
        },
        logout: {
          en: "Logout",
          es: "Cerrar sesión",
        },
        checkout: {
          en: "Checkout",
          es: "Caja",
        },
        phone: "+591 505 631288",
        logo: "https://res.cloudinary.com/ahossain/image/upload/v1697687802/settings/logo-light_hls14v.svg",
      },
      home: {
        coupon_status: true,
        featured_status: true,
        daily_needs_status: true,
        slider_width_status: false,
        promotion_banner_status: true,
        delivery_status: true,
        popular_products_status: true,
        discount_product_status: true,
        feature_promo_status: true,
        discount_coupon_code: ["SUMMER26", "WINTER25"],
        place_holder_img: "",
        discount_title: {
          en: "Latest Super Discount Active Coupon Code",
          es: "Último código de cupón de súper descuento activo",
        },
        promotion_title: {
          en: "100% Natural Quality Organic Product",
          es: "Producto de calidad biológica 100 % natural.",
        },
        promotion_description: {
          en: "See Our latest discounted products from here and get a special discount product",
          es: "Vea nuestros últimos productos con descuento desde aquí y obtenga un producto con descuento especial.",
        },
        promotion_button_name: {
          en: "Shop Now",
          es: "Compra ahora",
        },
        promotion_button_link: "/search?category=breakfast",
        feature_title: {
          en: "Featured Categories",
          es: "Categorías destacadas",
        },
        feature_description: {
          en: "Choose your necessary products from this feature categories.",
          es: "Elija los productos necesarios de estas categorías de funciones.",
        },
        feature_product_limit: 18,
        popular_title: {
          en: "Popular Products for Daily Shopping",
          es: "Productos populares para la compra diaria",
        },
        popular_description: {
          en: "See all our popular products in this week. You can choose your daily needs products from this list and get some special offer with free shipping.",
          es: "Descubre todos nuestros productos populares de esta semana. Elige tus productos diarios de esta lista y aprovecha una oferta especial con envío gratis.",
        },
        popular_product_limit: 18,
        quick_delivery_subtitle: {
          en: "Organic Products and Food",
          es: "Productos y alimentos orgánicos",
        },
        quick_delivery_title: {
          en: "Quick Delivery to Your Home",
          es: "Entrega rápida a su domicilio",
        },
        quick_delivery_description: {
          en: "There are many products you will find in our shop, Choose your daily necessary product from our KachaBazar shop and get some special offers. See Our latest discounted products from here and get a special discount.",
          es: "Encontrarás muchos productos en nuestra tienda. Elige tus productos diarios en KachaBazar y aprovecha ofertas especiales. Descubre nuestros últimos productos con descuento aquí y obtén un descuento especial.",
        },
        quick_delivery_button: {
          en: "Download App",
          es: "Descargar aplicación",
        },
        quick_delivery_link: "#",
        quick_delivery_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697688032/settings/delivery-boy_rluuoq.webp",
        latest_discount_title: {
          en: "Latest Discounted Products",
          es: "Últimos productos con descuento",
        },
        latest_discount_description: {
          en: "See Our latest discounted products below. Choose your daily needs from here and get a special discount with free shipping.",
          es: "Consulta nuestros últimos productos con descuento a continuación. Elige tus artículos diarios aquí y obtén un descuento especial con envío gratis.",
        },
        latest_discount_product_limit: 18,
        daily_need_title: {
          en: "Get Your Daily Needs From Our KachaBazar Store",
          es: "Obtenga sus necesidades diarias en nuestra tienda KachaBazar",
        },
        daily_need_description: {
          en: "There are many products you will find in our shop, Choose your daily necessary product from our KachaBazar shop and get some special offers.",
          es: "Encontrarás muchos productos en nuestra tienda. Elige tu producto diario necesario en nuestra tienda KachaBazar y obtén algunas ofertas especiales.",
        },
        daily_need_app_link: "https://www.apple.com/app-store/",
        daily_need_google_link:
          "https://play.google.com/store/games?utm_source=apac_med&utm_medium=hasem&utm_content=Jun0122&utm_campaign=Evergreen&pcampaignid=MKT-EDR-apac-lk-1003227-med-hasem-py-Evergreen-Jun0122-Text_Search_BKWS-BKWS%7CONSEM_kwid_43700071429441653_creativeid_600975795576_device_c&gclid=CjwKCAjwwo-WBhAMEiwAV4dybdy60tnQqCSnQ-cXShNnEcxmaBx2I6iwwc_WEqoA5sN9YSLJEXh9fBoC3u4QAvD_BwE&gclsrc=aw.ds",
        daily_need_img_left:
          "https://res.cloudinary.com/ahossain/image/upload/v1697688091/settings/app-download-img-left_s5n2zf.webp",
        daily_need_img_right:
          "https://res.cloudinary.com/ahossain/image/upload/v1697688091/settings/app-download-img_c7xqg4.webp",
        button1_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697688165/settings/app-store_cyyc0f.svg",
        button2_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697688167/settings/play-store_cavwua.svg",
      },
      about_us: {
        header_status: true,
        content_left_status: true,
        content_right_status: true,
        content_middle_status: true,
        founder_status: true,
        header_bg:
          "https://res.cloudinary.com/ahossain/image/upload/v1697439245/settings/yw3cd2xupqwqpqcbxv9l.jpg",
        content_right_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697439245/settings/v7g6gowiju0wanpwx70f.jpg",
        content_middle_Img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697439195/settings/sl8vzvzm54jgzq6sphn2.jpg",
        founder_one_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697439688/settings/team-1_acjmv7.webp",
        founder_two_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697439689/settings/team-2_dw7zs1.webp",
        founder_three_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697439689/settings/team-3_ld3323.webp",
        founder_four_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697439689/settings/team-4_i7jvx7.webp",
        founder_five_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697439689/settings/team-5_ylyklw.webp",
        founder_six_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697439689/settings/team-6_gmlts4.webp",
        title: {
          en: "About Us",
          es: "Sobre Nosotros",
        },
        top_title: {
          en: "Welcome to our KachaBazar shop",
          es: "Bienvenido a nuestra tienda KachaBazar",
        },
        top_description: {
          en: "Holisticly seize parallel metrics and functional ROI.Seamlessly revolutionize error-free internal or organic sources before effective scenarios. Progressively incentivize state of the art applications for efficient intellectual capital. Credibly leverage existing distinctive mindshare through cutting-edge schemas. Proactively procrastinate team building paradigms coordinate client-centric total transparent internal.\n\nDynamically embrace diverse customer service and installed base paradigms. Credibly seize enterprise-wide experiences for end-to-end data. Professionally brand flexible alignments and cost effective architectures. Enthusiastically incentivize seamless communities with seamlessly facilitate revolutionary metrics with strategic theme areas.",
          es: "Aproveche de forma holística las métricas paralelas y el ROI funcional. Revolucione fluidamente las fuentes internas u orgánicas sin errores antes de que se presenten escenarios efectivos. Incentive progresivamente las aplicaciones de vanguardia para un capital intelectual eficiente. Aproveche con credibilidad la reputación distintiva existente mediante esquemas de vanguardia. Procrastine proactivamente los paradigmas de trabajo en equipo y coordine una estrategia interna totalmente transparente y centrada en el cliente. Adopte dinámicamente diversos paradigmas de servicio al cliente y de base instalada. Aproveche con credibilidad las experiencias empresariales para obtener datos integrales. Identifique profesionalmente alineaciones flexibles y arquitecturas rentables. Incentive con entusiasmo las comunidades integradas facilitando fluidamente métricas revolucionarias con áreas temáticas estratégicas.",
        },
        card_one_title: {
          en: "10K",
          es: "10.000",
        },
        card_one_sub: {
          en: "Listed Products",
          es: "Productos listados",
        },
        card_one_description: {
          en: "Dynamically morph team driven partnerships after vertical",
          es: "Adapte dinámicamente las alianzas impulsadas por equipos según el sector",
        },
        card_two_title: {
          en: "8K",
          es: "8.000",
        },
        card_two_sub: {
          en: "Lovely Customer",
          es: "Clientes Encantados",
        },
        card_two_description: {
          en: "Competently productize virtual models without performance.",
          es: "Gestione de manera competente modelos virtuales sin pérdida de rendimiento.",
        },
        middle_description_one: {
          en: "Holisticly seize parallel metrics and functional ROI. Seamlessly revolutionize error-free internal or organic sources before effective scenarios. Progressively incentivize state of the art applications for efficient intellectual capital. Credibly leverage existing distinctive mindshare through cutting-edge schemas. Proactively procrastinate team building paradigms coordinate client-centric total transparent internal. Energistically reconceptualize global leadership for high-quality networks. Credibly restore an expanded array of systems rather than accurate results. Collaboratively synergize backend bandwidth without 24/7 functionalities. Credibly utilize proactive ideas whereas cross-media core competencies. Uniquely maximize professional best practices through resource maximizing services. Conveniently architect cross-unit web services for e-business imperatives.",
          es: "Aproveche de forma holística las métricas paralelas y el ROI funcional. Revolucione fluidamente las fuentes internas u orgánicas sin errores antes de que se presenten escenarios efectivos. Incentive progresivamente las aplicaciones de vanguardia para un capital intelectual eficiente. Aproveche con credibilidad la reputación distintiva existente mediante esquemas de vanguardia. Procrastine proactivamente los paradigmas de trabajo en equipo y coordine una transparencia interna total centrada en el cliente. Reconceptualice con energía el liderazgo global para redes de alta calidad. Restablezca con credibilidad una gama ampliada de sistemas en lugar de resultados precisos. Sinergice colaborativamente el ancho de banda del backend sin funcionalidades 24/7. Utilice con credibilidad ideas proactivas mientras que las competencias centrales intermedios. Maximice de forma única las mejores prácticas profesionales mediante servicios que optimizan los recursos. Diseñe convenientemente servicios web interunidades para las necesidades del e-business.",
        },
        middle_description_two: {
          en: "Appropriately visualize market-driven data before one-to-one scenarios. Collaboratively productize multifunctional ROI through intuitive supply chains. Enthusiastically seize revolutionary value and process-centric services. Competently harness intuitive information after interoperable markets. Interactively revolutionize future-proof value before granular sources. Dynamically embrace diverse customer service and installed base paradigms. Credibly seize enterprise-wide experiences for end-to-end data. Professionally brand flexible alignments and cost effective architectures. Enthusiastically incentivize seamless communities with seamlessly facilitate revolutionary metrics with strategic theme areas.",
          es: "Visualice adecuadamente los datos impulsados ​​por el mercado antes de escenarios individuales. Genere colaborativamente un ROI multifuncional mediante cadenas de suministro intuitivas. Aproveche con entusiasmo el valor revolucionario y los servicios centrados en procesos. Aproveche con competencia la información intuitiva tras la interoperabilidad de los mercados. Revolucione interactivamente el valor a futuro antes de las fuentes granulares. Adopte dinámicamente diversos paradigmas de servicio al cliente y base instalada. Aproveche con credibilidad las experiencias empresariales para obtener datos integrales. Identifique profesionalmente alineaciones flexibles y arquitecturas rentables. Incentive con entusiasmo comunidades integradas facilitando métricas revolucionarias con áreas temáticas estratégicas.",
        },
        founder_title: {
          en: "Our Team",
          es: "Nuestro Equipo",
        },
        founder_description: {
          en: "We’re impartial and independent, and every day we create distinctive, world-class reintermediate backend supply programmes.",
          es: "Somos imparciales e independientes y cada día creamos programas de suministro back-end reintermediados distintivos y de clase mundial.",
        },
        founder_one_name: {
          en: "Niamh Shea",
          de: "Niamh Shea ",
        },
        founder_one_sub: {
          en: "Co-founder & Executive",
          es: "Cofundador y Ejecutivo",
        },
        founder_two_name: {
          en: "Orla Dwyer",
          de: "Orla Dwyer ",
        },
        founder_two_sub: {
          en: "Orla Dwyer",
          de: "Orla Dwyer ",
        },
        founder_three_name: {
          en: "Danien James",
          de: "Danien James ",
        },
        founder_three_sub: {
          en: "Co-founder, Chairman",
          es: "Cofundador, presidente",
        },
        founder_four_name: {
          en: "Dara Frazier",
          de: "Dara Frazier ",
        },
        founder_four_sub: {
          en: "Chief Strategy Officer",
          es: "Directora de estrategia",
        },
        founder_five_name: {
          en: "Glenda Arvidson",
          de: "Glenda Arvidson",
        },
        founder_five_sub: {
          en: "HR Officer",
          es: "Oficial de RRHH",
        },
        founder_six_name: {
          en: "Melvin Davis ",
          de: "Melvin Davis ",
        },
        founder_six_sub: {
          en: "Lead Developer",
          es: "Desarrollador principal",
        },
      },
      contact_us: {
        header_status: true,
        email_box_status: true,
        call_box_status: true,
        address_box_status: true,
        left_col_status: true,
        form_status: true,
        header_bg:
          "https://res.cloudinary.com/ahossain/image/upload/v1697439245/settings/yw3cd2xupqwqpqcbxv9l.jpg",
        left_col_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697472924/settings/contact-us_zvwn2n.png",
        title: {
          en: "Contact Us",
          es: "Contacta con nosotros",
        },
        email_box_title: {
          en: "Email Us",
          es: "Envíenos un correo electrónico",
        },
        email_box_email: {
          en: "info@kachabazar.com ",
          de: "info@kachabazar.com",
        },
        email_box_text: {
          en: "Interactively grow empowered for process-centric total linkage. ",
          es: "Impulsa de manera interactiva un crecimiento empoderado para lograr una integración total orientada a los procesos.",
        },
        call_box_title: {
          en: "Call Us ",
          es: "Llámanos",
        },
        call_box_phone: {
          en: "029-00124667",
          de: "029-00124667",
        },
        call_box_text: {
          en: "Distinctively disseminate focused solutions clicks-and-mortar ministate. ",
          es: "Difunde de manera diferenciada soluciones enfocadas en un modelo híbrido físico-digital.",
        },
        address_box_title: {
          en: "Location ",
          es: "Ubicación",
        },
        address_box_address_one: {
          en: "Boho One, Bridge Street West, Middlesbrough, North Yorkshire, TS2 1AE.",
          es: "Boho One, Bridge Street West, Middlesbrough, North Yorkshire, TS2 1AE, Reino Unido.",
        },
        address_box_address_two: {
          en: "561-4535 Nulla LA ",
          es: "561-4535 Nulla, LA (Los Ángeles)",
        },
        address_box_address_three: {
          en: "United States 96522. ",
          es: "Estados Unidos 96522.",
        },
        form_title: {
          en: "For any suppoort just send your query ",
          es: "Para cualquier ayuda simplemente envíe su consulta.",
        },
        form_description: {
          en: "Collaboratively promote client-focused convergence vis-a-vis customer-directed alignments via plagiarized strategic users and standardized infrastructures. ",
          es: "Promover de manera colaborativa la convergencia centrada en el cliente en relación con alineamientos dirigidos al consumidor mediante usuarios estratégicos plagiados e infraestructuras estandarizadas.",
        },
      },
      offers: {
        header_status: true,
        header_bg:
          "https://res.cloudinary.com/ahossain/image/upload/v1697439245/settings/yw3cd2xupqwqpqcbxv9l.jpg",
        title: {
          en: "Mega Offer",
          es: "Mega Oferta",
        },
        coupon_code: ["SUMMER26", "WINTER25", "SUMMER24", "AUGUST24"],
      },
      privacy_policy: {
        status: true,
        header_bg:
          "https://res.cloudinary.com/ahossain/image/upload/v1697439245/settings/yw3cd2xupqwqpqcbxv9l.jpg",
        title: {
          en: "Privacy Policy",
          es: "Política de Privacidad",
        },
        description: {
          en: "<h1><strong>Last updated: February 15, 2022</strong></h1>\n<p>At KachaBazar, accessible from kachabazar dot com, one of our main priorities is the privacy of our visitors. This Privacy Policy document contains types of information that is collected and recorded by KachaBazar and how we use it. If you have additional questions or require more information about our Privacy Policy, do not hesitate to contact us.we may receive additional information about you such as your name, email address, phone number, the contents of the message and/or attachments you may send us, and any other information you may choose to provide. When you register for an Account, we may ask for your contact information, including items such as name, company name, address, email address, and telephone number.</p>\n<p>This Privacy Policy applies only to our online activities and is valid for visitors to our website with regards to the information that they shared and/or collect in KachaBazar.we may receive additional information about you such as your name, email address, phone number, the contents of the message and/or attachments you may send us, we may ask for your contact information, including items such as name, company name, address, email address, and telephone number. This policy is not applicable to any information collected offline or via channels other than this website. Our Privacy Policy was created with the help of the Free Privacy Policy Generator.</p>\n<h1><strong>Consent</strong></h1>\n<p>By using our website, you hereby consent to our Privacy Policy and agree to its terms.</p>\n<h1><strong>Information we collect</strong></h1>\n<p>The personal information that you are asked to provide, and the reasons why you are asked to provide it, will be made clear to you at the point we ask you to provide your personal information. If you contact us directly, we may receive additional information about you such may choose to provide. When you register for an Account, we may ask for your contact information, including items such as name, company name, address, email address, and telephone number.</p>\n<p>Please note that the Company will not ask you to share any sensitive data or information via email or telephone. If you receive any such request by email or telephone, please do not respond/divulge any sensitive data or information and forward the information relating to the same to</p>\n<h1><strong>How we use your information</strong></h1>\n<p>We use the information we collect in various ways, including to:</p>\n<ol>\n<li>1. Provide, operate, and maintain our website, to provide you with updates and other information.</li>\n<li>2. Improve, personalize, and expand our website,and other information relating to the website.</li>\n<li>3. Understand and analyze how you use our website, to provide you with updates and other information relating to the website.</li>\n<li>4. Develop new products, services, features, and functionality,and other information relating to the website.</li>\n<li>5. Communicate with you, either directly or through one of our partners, including for customer service, to provide you with updates.</li>\n<li>6. Send you emails. To provide you with updates and other information relating to the website, and for marketing and promotional purposes</li>\n<li>7. Find and prevent fraud. To provide you with updates and other information relating to the website, and for marketing and promotional purposes</li>\n</ol>\n<h1><strong>Log Files</strong></h1>\n<p>KachaBazar follows a standard procedure of using log files. These files log visitors when they visit websites. All hosting companies do this and a part of hosting. The information collected by log files include internet protocol (IP) addresses, browser type, Internet Service Provider (ISP), date and time stamp, referring/exit pages, and possibly the number of clicks. These are not linked to any information that is personally identifiable. The purpose of the information is for analyzing trends, administering the site, tracking users' movement on the website, and gathering demographic information.</p>\n<h1><strong>Advertising Partners Privacy Policies</strong></h1>\n<p>You may consult this list to find the Privacy Policy for each of the advertising partners of KachaBazar. Third-party ad servers or ad networks uses technologies like cookies, JavaScript, or Web Beacons that are used in their respective advertisements and links that appear on KachaBazar, which are sent directly to users' browser. They automatically receive your IP address when this occurs. These technologies are used to measure the effectiveness of their advertising campaigns and/or to personalize the advertising content that you see on websites that you visit. Note that KachaBazar has no access to or control over these cookies that are used by third-party advertisers.</p>\n<h1><strong>Third Party Privacy Policies</strong></h1>\n<p>Third-party ad servers or ad networks uses technologies like cookies, JavaScript, or Web Beacons that are used in their respective advertisements and links that appear on KachaBazar, which are sent directly to users' browser. They automatically receive your IP address when this occurs. These technologies are used to measure the effectiveness of their advertising campaigns and/or to personalize the advertising content that you see on websites that you visit. Note that KachaBazar has no access to or control over these cookies that are used by third-party advertisers.</p>\n<p>KachaBazar's Privacy Policy does not apply to other advertisers or websites. Thus, we are advising you to consult the respective Privacy Policies of these third-party ad servers for more detailed information. It may include their practices and instructions about how to opt-out of certain options. You can choose to disable cookies through your individual browser options. To know more detailed information about cookie management with specific web browsers, it can be found at the browsers' respective websites.</p>\n<h1><strong>CCPA Privacy Rights</strong></h1>\n<p>Under the CCPA, among other rights, California consumers have the right to: Request that a business that collects a consumer's personal data disclose the categories and specific pieces of personal data that a business has collected about consumers. Request that a business delete any personal data about the consumer that a business has collected. Request that a business that sells a consumer's personal data, not sell the consumer's personal data. If you make a request, we have one month to respond to you. If you would like to exercise any of these rights, please contact us.</p>\n<h1><strong>Children's Information</strong></h1>\n<p>Another part of our priority is adding protection for children while using the internet. We encourage parents and guardians to observe, participate in, and/or monitor and guide their online activity.Request that a business delete any personal data about the consumer that a business has collected. If you make a request, we have one month to respond to you. If you would like to exercise any of these rights, please contact us.</p>\n<p>KachaBazar does not knowingly collect any Personal Identifiable Information from children under the age of 13. If you think that your child provided this kind of information on our website, we strongly encourage you to contact us immediately and we will do our best efforts to promptly remove such information from our records.</p>\n",
          es: "<h1><strong>Última actualización: 15 de febrero de 2022</strong></h1>\n<p>En KachaBazar, accesible desde kachabazar dot com, una de nuestras principales prioridades es la privacidad de nuestros visitantes. Este documento de Política de Privacidad contiene los tipos de información que KachaBazar recopila y registra y cómo la utilizamos. Si tienes preguntas adicionales o necesitas más información sobre nuestra Política de Privacidad, no dudes en contactarnos. Podemos recibir información adicional sobre ti, como tu nombre, dirección de correo electrónico, número de teléfono, el contenido del mensaje y/o archivos adjuntos que nos envíes, y cualquier otra información que decidas proporcionar. Cuando te registras para una Cuenta, podemos pedir tu información de contacto, incluidos datos como nombre, nombre de la empresa, dirección, correo electrónico y número de teléfono.</p>\n<p>Esta Política de Privacidad se aplica únicamente a nuestras actividades en línea y es válida para los visitantes de nuestro sitio web en relación con la información que compartieron y/o recopilaron en KachaBazar. Podemos recibir información adicional sobre ti, como tu nombre, dirección de correo electrónico, número de teléfono, el contenido del mensaje y/o archivos adjuntos que nos envíes. Podemos pedir tu información de contacto, incluidos datos como nombre, nombre de la empresa, dirección, correo electrónico y número de teléfono. Esta política no es aplicable a ninguna información recopilada fuera de línea o a través de canales distintos a este sitio web. Nuestra Política de Privacidad fue creada con la ayuda del Generador Gratuito de Políticas de Privacidad.</p>\n<h1><strong>Consentimiento</strong></h1>\n<p>Al utilizar nuestro sitio web, otorgas tu consentimiento a nuestra Política de Privacidad y aceptas sus términos.</p>\n<h1><strong>Información que recopilamos</strong></h1>\n<p>La información personal que se te pida proporcionar, y las razones por las que se te solicite, se te aclararán en el momento en que te pidamos tu información personal. Si nos contactas directamente, podemos recibir información adicional sobre ti que decidas proporcionar. Cuando te registras para una Cuenta, podemos pedir tu información de contacto, incluidos datos como nombre, nombre de la empresa, dirección, correo electrónico y número de teléfono.</p>\n<p>Ten en cuenta que la Compañía no te pedirá compartir ningún dato o información sensible por correo electrónico o teléfono. Si recibes alguna solicitud de este tipo por correo electrónico o teléfono, no respondas ni divulgues información sensible y reenvía dicha información relacionada con la solicitud a nosotros.</p>\n<h1><strong>Cómo usamos tu información</strong></h1>\n<p>Usamos la información que recopilamos de diversas maneras, incluyendo para:</p>\n<ol>\n<li>1. Proporcionar, operar y mantener nuestro sitio web, brindándote actualizaciones y otra información.</li>\n<li>2. Mejorar, personalizar y ampliar nuestro sitio web, y otra información relacionada con el mismo.</li>\n<li>3. Comprender y analizar cómo utilizas nuestro sitio web, para brindarte actualizaciones y otra información relacionada con el mismo.</li>\n<li>4. Desarrollar nuevos productos, servicios, características y funcionalidades, y otra información relacionada con el sitio web.</li>\n<li>5. Comunicarnos contigo, ya sea directamente o a través de uno de nuestros socios, incluyendo atención al cliente, para brindarte actualizaciones.</li>\n<li>6. Enviarte correos electrónicos para proporcionarte actualizaciones y otra información relacionada con el sitio web, así como con fines de marketing y promoción.</li>\n<li>7. Detectar y prevenir fraudes, proporcionándote actualizaciones y otra información relacionada con el sitio web, así como con fines de marketing y promoción.</li>\n</ol>\n<h1><strong>Archivos de registro</strong></h1>\n<p>KachaBazar sigue un procedimiento estándar de uso de archivos de registro. Estos archivos registran a los visitantes cuando visitan sitios web. Todas las empresas de alojamiento hacen esto como parte de sus servicios de hosting. La información recopilada por los archivos de registro incluye direcciones de protocolo de Internet (IP), tipo de navegador, proveedor de servicios de Internet (ISP), fecha y hora, páginas de referencia/salida y, posiblemente, el número de clics. Estos datos no están vinculados a ninguna información que sea personalmente identificable. El propósito de esta información es analizar tendencias, administrar el sitio, rastrear el movimiento de los usuarios en el sitio web y recopilar información demográfica.</p>\n<h1><strong>Políticas de privacidad de socios publicitarios</strong></h1>\n<p>Puedes consultar esta lista para encontrar la Política de Privacidad de cada uno de los socios publicitarios de KachaBazar. Los servidores o redes publicitarias de terceros utilizan tecnologías como cookies, JavaScript o balizas web en sus respectivos anuncios y enlaces que aparecen en KachaBazar, los cuales se envían directamente al navegador de los usuarios. Automáticamente reciben tu dirección IP cuando esto ocurre. Estas tecnologías se utilizan para medir la efectividad de sus campañas publicitarias y/o personalizar el contenido publicitario que ves en los sitios web que visitas. Ten en cuenta que KachaBazar no tiene acceso ni control sobre estas cookies utilizadas por anunciantes de terceros.</p>\n<h1><strong>Políticas de privacidad de terceros</strong></h1>\n<p>Los servidores o redes publicitarias de terceros utilizan tecnologías como cookies, JavaScript o balizas web en sus respectivos anuncios y enlaces que aparecen en KachaBazar, los cuales se envían directamente al navegador de los usuarios. Automáticamente reciben tu dirección IP cuando esto ocurre. Estas tecnologías se utilizan para medir la efectividad de sus campañas publicitarias y/o personalizar el contenido publicitario que ves en los sitios web que visitas. Ten en cuenta que KachaBazar no tiene acceso ni control sobre estas cookies utilizadas por anunciantes de terceros.</p>\n<p>La Política de Privacidad de KachaBazar no se aplica a otros anunciantes o sitios web. Por lo tanto, te aconsejamos consultar las Políticas de Privacidad correspondientes de estos servidores de anuncios de terceros para obtener información más detallada. Esto puede incluir sus prácticas e instrucciones sobre cómo excluirte de ciertas opciones. Puedes optar por deshabilitar las cookies desde las opciones de tu navegador. Para obtener más información sobre la gestión de cookies en navegadores específicos, puedes consultar los sitios web de cada navegador.</p>\n<h1><strong>Derechos de privacidad bajo la CCPA</strong></h1>\n<p>Bajo la CCPA, entre otros derechos, los consumidores de California tienen derecho a: Solicitar que una empresa que recopile datos personales de un consumidor divulgue las categorías y piezas específicas de datos personales que ha recopilado sobre los consumidores. Solicitar que una empresa elimine cualquier dato personal del consumidor que haya recopilado. Solicitar que una empresa que venda datos personales de un consumidor no venda dicha información. Si realizas una solicitud, tenemos un mes para responderte. Si deseas ejercer cualquiera de estos derechos, por favor contáctanos.</p>\n<h1><strong>Información de los niños</strong></h1>\n<p>Otra parte de nuestra prioridad es brindar protección a los niños mientras usan internet. Animamos a los padres y tutores a observar, participar y/o monitorear y guiar su actividad en línea. Solicitar que una empresa elimine cualquier dato personal del consumidor que haya recopilado. Si realizas una solicitud, tenemos un mes para responderte. Si deseas ejercer cualquiera de estos derechos, por favor contáctanos.</p>\n<p>KachaBazar no recopila de manera consciente ninguna Información Personal Identificable de niños menores de 13 años. Si crees que tu hijo proporcionó este tipo de información en nuestro sitio web, te alentamos a contactarnos de inmediato y haremos todo lo posible para eliminar dicha información de nuestros registros lo antes posible.</p>\n",
        },
      },
      term_and_condition: {
        status: true,
        header_bg:
          "https://res.cloudinary.com/ahossain/image/upload/v1697439245/settings/yw3cd2xupqwqpqcbxv9l.jpg",
        title: {
          en: "Terms & Conditions",
          es: "Términos y condiciones",
        },
        description: {
          en: '<h1><strong>Welcome to KachaBazar!</strong></h1>\n<p>These terms and conditions outline the rules and regulations for the use of KachaBazar\'s Website, located at https://kachabazar.com/. By accessing this website we assume you accept these terms and conditions. Do not continue to use KachaBazar if you do not agree to take all of the terms and conditions stated on this page.</p>\n<p>The following terminology applies to these Terms and Conditions, Privacy Statement and Disclaimer Notice and all Agreements: Client, You and Your refers to you, the person log on this website and compliant to the Company’s terms and conditions. The Company, refers to our CompanyPartParties or Us refers to both the Client and ourselves. All terms refer to the offer, acceptance and consideration of payment necessary to undertake the process of our assistance to the Client in the most appropriate manner for the express purpose of meeting the Client’s needs in respect of provision of the Company’s stated services, in accordance with and subject to, prevailing law of Netherlands. Any use of the above terminology or other words in the singular, plural, capitalization and/or he/she or they, are taken as interchangeable and therefore as referring to same.</p>\n<h1><strong>Cookies</strong></h1>\n<p>We employ the use of cookies. By accessing KachaBazar, you agreed to use cookies in agreement with the KachaBazar\'s Privacy Policy. Most interactive websites use cookies to let us retrieve the user’s details for each visit. Cookies are used by our website to enable the functionality of certain areas to make it easier for people visiting our website. Some of our affiliate/advertising partners may also use cookies.</p>\n<h1><strong>License</strong></h1>\n<p>Unless otherwise stated, KachaBazar and/or its licensors own the intellectual property rights for all material on KachaBazar. All intellectual property rights are reserved. You may access this from KachaBazar for your own personal use subjected to restrictions set in these terms and conditions.This Agreement shall begin on the date hereof. Our Terms and Conditions were created with the help of the Terms And Conditions Generator.<strong>You must not:</strong></p>\n<ul>\n<li>1. Identifiers (e.g. name, mailing address, email address, phone number, credit/debit card number)</li>\n<li>2. Characteristics of protected classifications (e.g. gender, age)</li>\n<li>3. Commercial information (e.g. products or services purchased, purchase history)</li>\n<li>4. Internet or other electronic network activity (e.g. browse or search history)</li>\n<li>5. Geo location data (e.g. latitude or longitude)</li>\n<li>6. Audio, electronic, visual, or similar information (e.g. recording of Guest service calls)</li>\n<li>7. Inferences drawn from any of the above (e.g. preferences or characteristics)</li>\n</ul>\n<p>Parts of this website offer an opportunity for users to post and exchange opinions and information in certain areas of the website. KachaBazar does not filter, edit, publish or review Comments prior to their presence on the website. Comments do not reflect the views and opinions of KachaBazar,its agents and/or affiliates. Comments reflect the views and opinions of the person who post their views and opinions. To the extent permitted by applicable laws, KachaBazar shall not be liable for the Comments or for any liability, damages or expenses caused and/or suffered as a result of any use of and/or posting of and/or appearance of the Comments on this website.</p>\n<h1><strong>Content Liability</strong></h1>\n<p>We shall not be hold responsible for any content that appears on your Website. You agree to protect and defend us against all claims that is rising on your Website. No link(s) should appear on any Website that may be interpreted as libelous, obscene or criminal, or which infringes, otherwise violates, or advocates the infringement or other violation of, any third party rights.Without prior approval and written permission, you may not create frames around our Webpages that alter in any way the visual presentation or appearance of our Website.</p>\n<h1><strong>Your Privacy</strong></h1>\n<p>Please read <a href="https://kachabazar-store.vercel.app/privacy-policy" target="_self">Privacy Policy</a></p>\n<h1><strong>Reservation of Rights</strong></h1>\n<p>We reserve the right to request that you remove all links or any particular link to our Website. You approve to immediately remove all links to our Website upon request. We also reserve the right to amen these terms and conditions and it’s linking policy at any time. By continuously linking to our Website, you agree to be bound to and follow these linking terms and conditions.</p>\n<h1><strong>Disclaimer</strong></h1>\n<p>To the maximum extent permitted by applicable law, we exclude all representations, warranties and conditions relating to our website and the use of this website. Nothing in this disclaimer will:</p>\n<ul>\n<li>1. limit or exclude our or your liability for death or personal injury;</li>\n<li>2. limit or exclude our or your liability for fraud or fraudulent misrepresentation;</li>\n<li>3. limit any of our or your liabilities in any way that is not permitted under applicable law; or</li>\n<li>4. exclude any of our or your liabilities that may not be excluded under applicable law.</li>\n</ul>\n<p>The limitations and prohibitions of liability set in this Section and elsewhere in this disclaimer: (a) are subject to the preceding paragraph; and (b) govern all liabilities arising under the disclaimer, including liabilities arising in contract, in tort and for breach of statutory duty. As long as the website and the information and services on the website are provided free of charge, we will not be liable for any loss or damage of any nature.</p>\n',
          es: '<h1><strong>¡Bienvenido a KachaBazar!</strong></h1>\n<p>Estos términos y condiciones describen las reglas y regulaciones para el uso del sitio web de KachaBazar, ubicado en https://kachabazar.com/. Al acceder a este sitio web, asumimos que aceptas estos términos y condiciones. No continúes usando KachaBazar si no estás de acuerdo con todos los términos y condiciones establecidos en esta página.</p>\n<p>La siguiente terminología se aplica a estos Términos y Condiciones, Declaración de Privacidad y Aviso Legal y a todos los Acuerdos: Cliente, Tú y Tu se refieren a ti, la persona que inicia sesión en este sitio web y cumple con los términos y condiciones de la Compañía. La Compañía se refiere a nuestra empresa. Partes o Nosotros se refieren tanto al Cliente como a nosotros mismos. Todos los términos se refieren a la oferta, aceptación y consideración del pago necesario para llevar a cabo el proceso de nuestra asistencia al Cliente de la manera más adecuada con el propósito expreso de satisfacer las necesidades del Cliente con respecto a la provisión de los servicios establecidos de la Compañía, de acuerdo con y sujeto a la legislación vigente de los Países Bajos. Cualquier uso de la terminología anterior u otras palabras en singular, plural, mayúsculas y/o él/ella o ellos, se consideran intercambiables y, por lo tanto, se refieren a lo mismo.</p>\n<h1><strong>Cookies</strong></h1>\n<p>Utilizamos cookies. Al acceder a KachaBazar, aceptas el uso de cookies de acuerdo con la Política de Privacidad de KachaBazar. La mayoría de los sitios web interactivos utilizan cookies para permitirnos recuperar los datos del usuario en cada visita. Nuestro sitio web utiliza cookies para habilitar la funcionalidad de ciertas áreas y hacer que sea más fácil para los visitantes. Algunos de nuestros socios afiliados/de publicidad también pueden usar cookies.</p>\n<h1><strong>Licencia</strong></h1>\n<p>A menos que se indique lo contrario, KachaBazar y/o sus licenciantes poseen los derechos de propiedad intelectual de todo el material en KachaBazar. Todos los derechos de propiedad intelectual están reservados. Puedes acceder a este contenido desde KachaBazar para tu propio uso personal sujeto a las restricciones establecidas en estos términos y condiciones. Este Acuerdo comenzará en la fecha actual. Nuestros Términos y Condiciones fueron creados con la ayuda del Generador de Términos y Condiciones.<strong>No debes:</strong></p>\n<ul>\n<li>1. Identificadores (ej. nombre, dirección postal, correo electrónico, número de teléfono, número de tarjeta de crédito/débito)</li>\n<li>2. Características de clasificaciones protegidas (ej. género, edad)</li>\n<li>3. Información comercial (ej. productos o servicios comprados, historial de compras)</li>\n<li>4. Actividad en Internet u otra red electrónica (ej. historial de navegación o búsqueda)</li>\n<li>5. Datos de geolocalización (ej. latitud o longitud)</li>\n<li>6. Información de audio, electrónica, visual o similar (ej. grabaciones de llamadas de servicio al cliente)</li>\n<li>7. Inferencias derivadas de cualquiera de los anteriores (ej. preferencias o características)</li>\n</ul>\n<p>Algunas partes de este sitio web ofrecen la oportunidad a los usuarios de publicar e intercambiar opiniones e información en ciertas áreas del sitio. KachaBazar no filtra, edita, publica ni revisa los Comentarios antes de su aparición en el sitio web. Los Comentarios no reflejan las opiniones de KachaBazar, sus agentes y/o afiliados. Los Comentarios reflejan las opiniones de la persona que los publica. En la medida permitida por las leyes aplicables, KachaBazar no será responsable de los Comentarios ni de cualquier responsabilidad, daños o gastos causados y/o sufridos como resultado del uso, publicación y/o aparición de los Comentarios en este sitio web.</p>\n<h1><strong>Responsabilidad sobre el Contenido</strong></h1>\n<p>No seremos responsables de ningún contenido que aparezca en tu sitio web. Aceptas protegernos y defendernos contra todas las reclamaciones que surjan en tu sitio web. Ningún enlace debe aparecer en ningún sitio web que pueda interpretarse como difamatorio, obsceno o criminal, o que infrinja, viole o promueva la infracción de derechos de terceros. Sin aprobación previa y permiso por escrito, no puedes crear marcos alrededor de nuestras páginas web que alteren de ninguna manera la presentación visual o la apariencia de nuestro sitio web.</p>\n<h1><strong>Tu Privacidad</strong></h1>\n<p>Por favor lee la <a href="https://kachabazar-store.vercel.app/privacy-policy" target="_self">Política de Privacidad</a></p>\n<h1><strong>Reserva de Derechos</strong></h1>\n<p>Nos reservamos el derecho de solicitar que elimines todos los enlaces o cualquier enlace en particular a nuestro sitio web. Aceptas eliminar de inmediato todos los enlaces a nuestro sitio web a solicitud nuestra. También nos reservamos el derecho de modificar estos términos y condiciones y su política de enlaces en cualquier momento. Al continuar enlazando a nuestro sitio web, aceptas estar sujeto y seguir estos términos y condiciones de enlace.</p>\n<h1><strong>Descargo de Responsabilidad</strong></h1>\n<p>En la máxima medida permitida por la ley aplicable, excluimos todas las representaciones, garantías y condiciones relacionadas con nuestro sitio web y el uso de este. Nada en este descargo de responsabilidad podrá:</p>\n<ul>\n<li>1. limitar o excluir nuestra responsabilidad o la tuya por muerte o lesiones personales;</li>\n<li>2. limitar o excluir nuestra responsabilidad o la tuya por fraude o tergiversación fraudulenta;</li>\n<li>3. limitar cualquiera de nuestras responsabilidades o las tuyas de manera no permitida por la ley aplicable; o</li>\n<li>4. excluir cualquiera de nuestras responsabilidades o las tuyas que no puedan excluirse bajo la ley aplicable.</li>\n</ul>\n<p>Las limitaciones y prohibiciones de responsabilidad establecidas en esta Sección y en otras partes de este descargo de responsabilidad: (a) están sujetas al párrafo anterior; y (b) rigen todas las responsabilidades que surjan bajo este descargo de responsabilidad, incluidas las responsabilidades contractuales, extracontractuales y por incumplimiento de deber legal. Mientras el sitio web y la información y servicios en él se proporcionen de forma gratuita, no seremos responsables de ninguna pérdida o daño de cualquier naturaleza.</p>\n',
        },
      },
      faq: {
        page_status: true,
        leftcol_status: true,
        rightcol_status: true,
        header_bg:
          "https://res.cloudinary.com/ahossain/image/upload/v1697439245/settings/yw3cd2xupqwqpqcbxv9l.jpg",
        left_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697687955/settings/faq_qr1y1h.svg",
        title: {
          en: "FAQs",
          es: "Preguntas frecuentes",
        },
        faq_one: {
          en: "How does the KachaBazar work?",
          es: "¿Cómo funciona el KachaBazar?",
        },
        description_one: {
          en: "Distinctively initiate error-free channels with highly efficient ROI. Intrinsicly envisioneer world-class data via best-of-breed best practices. Efficiently enable empowered e-tailers after cross-unit services. Uniquely expedite seamless e-tailers via cooperative interfaces. Monotonectally myocardinate customer directed meta-services whereas error-free scenarios.",
          es: "Implemente canales confiables que garanticen un alto retorno de inversión. Gestione datos de nivel mundial aplicando las mejores prácticas. Potencie el comercio electrónico mediante servicios integrados entre áreas. Optimice las operaciones de e-commerce con interfaces colaborativas y fluidas. Ofrezca servicios personalizados al cliente asegurando procesos sin errores.",
        },
        faq_two: {
          en: "Can I cancel my subscription anytime?",
          es: "¿Puedo cancelar mi suscripción en cualquier momento?",
        },
        description_two: {
          en: "Yes. You can cancel your subscription anytime. Your subscription will continue to be active until the end of your current term (month or year) but it will not auto-renew. Unless you delete your account manually, your account and all data will be deleted 60 days from the day your subscription becomes inactive.",
          es: "Sí. Puedes cancelar tu suscripción en cualquier momento. Tu suscripción seguirá activa hasta el final del período actual (mes o año), pero no se renovará automáticamente. A menos que elimines tu cuenta manualmente, tu cuenta y todos tus datos se eliminarán 60 días después de que tu suscripción quede inactiva.",
        },
        faq_three: {
          en: "Whice payment method you should accept?",
          es: "¿Qué método de pago debería aceptar?",
        },
        description_three: {
          en: "Holisticly engage sticky niche markets before collaborative collaboration and idea-sharing. Phosfluorescently facilitate parallel applications with unique imperatives. Proactively plagiarize functionalized deliverables via inexpensive solutions. Collaboratively embrace web-enabled infomediaries rather than diverse testing procedures.",
          es: "Conecte de forma integral con mercados de nicho antes de implementar procesos colaborativos y de intercambio de ideas. Impulse aplicaciones paralelas con enfoques únicos e innovadores. Aproveche soluciones económicas para optimizar la entrega de resultados funcionales. Fomente el uso de plataformas digitales colaborativas en lugar de procesos de prueba dispersos.",
        },
        faq_four: {
          en: "Can I cancel my subscription anytime?",
          es: "¿Puedo cancelar mi suscripción en cualquier momento?",
        },
        description_four: {
          en: "Continually impact seamless imperatives for best-of-breed best practices. Phosfluorescently facilitate parallel applications with unique imperatives. Proactively plagiarize functionalized deliverables via inexpensive solutions. Collaboratively embrace web-enabled infomediaries rather than diverse testing procedures.",
          es: "Genere un impacto constante aplicando prácticas óptimas y efectivas. Impulse aplicaciones paralelas con enfoques únicos e innovadores. Optimice la entrega de resultados mediante soluciones rentables y funcionales. Fomente el uso de plataformas digitales colaborativas en lugar de métodos de prueba dispersos.",
        },
        faq_five: {
          en: "What is KachaBazar EC2 auto scaling?",
          es: "¿Qué es el escalado automático de KachaBazar EC2??",
        },
        description_five: {
          en: "Continually impact seamless imperatives for best-of-breed best practices. Phosfluorescently facilitate parallel applications with unique imperatives. Proactively plagiarize functionalized deliverables via inexpensive solutions. Collaboratively embrace web-enabled infomediaries rather than diverse testing procedures.",
          es: "Genere un impacto constante aplicando las mejores prácticas del sector. Impulse aplicaciones paralelas con estrategias únicas e innovadoras. Optimice la entrega de resultados mediante soluciones funcionales y rentables. Fomente el uso de plataformas digitales colaborativas en lugar de procesos de prueba dispersos.",
        },
        faq_six: {
          en: "What are the benefits of using KachaBazar affliate?",
          es: "¿Cuáles son los beneficios de utilizar el programa de afiliados de KachaBazar?",
        },
        description_six: {
          en: "Continually impact seamless imperatives for best-of-breed best practices. Phosfluorescently facilitate parallel applications with unique imperatives. Proactively plagiarize functionalized deliverables via inexpensive solutions. Collaboratively embrace web-enabled infomediaries rather than diverse testing procedures.",
          es: "Genere un impacto constante aplicando las mejores prácticas del sector. Impulse aplicaciones paralelas con enfoques innovadores y únicos. Optimice la entrega de resultados mediante soluciones rentables y funcionales. Fomente plataformas digitales colaborativas en lugar de procesos de prueba dispersos.",
        },
        faq_seven: {
          en: "What is a affliates product configuration?",
          es: "¿Qué es una configuración de producto de afiliados?",
        },
        description_seven: {
          en: "Yes. You can cancel your subscription anytime. Your subscription will continue to be active until the end of your current term (month or year) but it will not auto-renew. Unless you delete your account manually, your account and all data will be deleted 60 days from the day your subscription becomes inactive.",
          es: "Sí. Puedes cancelar tu suscripción en cualquier momento. Tu suscripción continuará activa hasta el final del período actual (mes o año), pero no se renovará automáticamente. A menos que elimines tu cuenta manualmente, tu cuenta y todos tus datos se eliminarán 60 días después de que tu suscripción quede inactiva.",
        },
        faq_eight: {
          en: "What is fleet management and how is it different from dynamic scaling?",
          es: "¿Qué es la gestión de flotas y en qué se diferencia del escalamiento dinámico?",
        },
        description_eight: {
          en: "Distinctively initiate error-free channels with highly efficient ROI. Intrinsicly envisioneer world-class data via best-of-breed best practices. Efficiently enable empowered e-tailers after cross-unit services. Uniquely expedite seamless e-tailers via cooperative interfaces. Monotonectally myocardinate customer directed meta-services whereas error-free scenarios.",
          es: "Implemente canales confiables que garanticen un alto retorno de inversión. Gestione datos de nivel mundial aplicando las mejores prácticas del sector. Potencie a los minoristas online mediante servicios integrados entre áreas. Optimice la experiencia de e-commerce con interfaces colaborativas y sin interrupciones. Ofrezca servicios personalizados al cliente asegurando procesos sin errores.",
        },
      },
      slider: {
        left_right_arrow: false,
        bottom_dots: true,
        both_slider: false,
        first_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697688491/settings/slider-1_rl8qdc.jpg",
        first_title: {
          en: "The Best Quality Products Guaranteed!",
          es: "¡Productos de la mejor calidad garantizada!",
        },
        first_description: {
          en: "The Best Quality Products Guaranteed!",
          es: "¡Productos de la mejor calidad garantizada!",
        },
        first_button: {
          en: "Shop Now",
          es: "Compra ahora",
        },
        first_link: "/search?category=milk-dairy",
        second_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697688491/settings/slider-2_o6aezc.jpg",
        second_title: {
          en: "Best Different Type of Grocery Store",
          es: "Los mejores y diferentes tipos de tiendas de comestibles",
        },
        second_description: {
          en: "Quickly aggregate empowered networks after emerging products...",
          es: "Agregue rápidamente redes empoderadas luego de productos emergentes...",
        },
        second_button: {
          en: "Shop Now",
          es: "Compra ahora",
        },
        second_link: "/search?category=fish-meat",
        third_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697688492/settings/slider-3_iw4nnf.jpg",
        third_title: {
          en: "Quality Freshness Guaranteed!",
          es: "¡Frescura de calidad garantizada!",
        },
        third_description: {
          en: "Intrinsicly fashion performance based products rather than accurate benefits...",
          es: "Productos de rendimiento de moda intrínsecamente basados ​​en beneficios precisos...",
        },
        third_button: {
          en: "Shop Now",
          es: "Compra ahora",
        },
        third_link: "/search?category=fruits-vegetable",
        four_img: "https://i.postimg.cc/rscqZJNz/slider-1.webp",
        four_title: {
          en: "The Best Quality Products Guaranteed!",
          es: "¡Productos de la mejor calidad garantizada!",
        },
        four_description: {
          en: "Dramatically facilitate effective total linkage for go forward processes...",
          es: "Facilitar dramáticamente la vinculación total efectiva para avanzar en los procesos...",
        },
        four_button: {
          en: "Shop Now",
          es: "Compra ahora",
        },
        four_link: "/search?category=fruits-vegetable",
        five_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697688491/settings/slider-2_o6aezc.jpg",
        five_title: {
          en: "Best Different Type of Grocery Store",
          es: "Los mejores y diferentes tipos de tiendas de comestibles",
        },
        five_description: {
          en: "Quickly aggregate empowered networks after emerging products...",
          es: "Agregue rápidamente redes empoderadas luego de productos emergentes...",
        },
        five_button: {
          en: "Shop Now",
          es: "Compra ahora",
        },
        five_link: "/search?category=fish-meat",
      },

      checkout: {
        personal_details: {
          en: "Personal Details",
          es: "Datos personales",
        },
        first_name: {
          en: "First Name",
          es: "Nombre",
        },
        last_name: {
          en: "Last Name",
          es: "Apellido",
        },
        email_address: {
          en: "Email Address",
          es: "Dirección de correo electrónico",
        },
        checkout_phone: {
          en: "Phone Number",
          es: "Número de teléfono",
        },
        shipping_details: {
          en: "Shipping Details",
          es: "Detalles de envío",
        },
        street_address: {
          en: "Street Address",
          es: "Dirección de la calle",
        },
        city: {
          en: "City",
          es: "Ciudad",
        },
        country: {
          en: "Country",
          es: "País",
        },
        zip_code: {
          en: "Zip Code",
          es: "Código postal",
        },
        shipping_cost: {
          en: "Shipping Cost",
          es: "Costo de envío",
        },
        shipping_name_one: {
          en: "FedEx",
          es: "FedEx",
        },
        shipping_one_cost: 60,
        shipping_one_desc: {
          en: "Delivery: Today Cost :",
          es: "Entrega: Hoy Costo:",
        },
        shipping_name_two: {
          en: "UPS",
          es: "UPS",
        },
        shipping_two_desc: {
          en: "Delivery: 7 Days Cost :",
          es: "Entrega: 7 días Costo:",
        },
        shipping_two_cost: 20,
        payment_method: {
          en: "Payment Method",
          es: "Método de pago",
        },
        continue_button: {
          en: "Continue Shipping",
          es: "Continuar envío",
        },
        confirm_button: {
          en: "Confirm Order",
          es: "Confirmar pedido",
        },
        order_summary: {
          en: "Order Summary",
          es: "Resumen del pedido",
        },
        apply_button: {
          en: "Apply",
          es: "Aplicar",
        },
        sub_total: {
          en: "Sub total",
          es: "Sub total",
        },
        discount: {
          en: "Discount",
          es: "Descuento",
        },
        total_cost: {
          en: "TOTAL COST",
          es: "COSTO TOTAL",
        },
      },

      dashboard: {
        invoice_message_first: {
          en: "Thank You",
          es: "Gracias",
        },
        invoice_message_last: {
          en: "Your order have been received !",
          es: "¡Su pedido ha sido recibido!",
        },
        print_button: {
          en: "Print Invoice",
          es: "Imprimir factura",
        },
        download_button: {
          en: "Download Invoice",
          es: "Descargar factura",
        },
        dashboard_title: {
          en: "Dashboard",
          es: "Panel de control",
        },
        total_order: {
          en: "Total Orders",
          es: "Total de pedidos",
        },
        pending_order: {
          en: "Pending Orders",
          es: "Pedidos pendientes",
        },
        processing_order: {
          en: "Orders in processing",
          es: "Pedidos en proceso",
        },
        complete_order: {
          en: "Complete Orders",
          es: "Pedidos completados",
        },
        recent_order: {
          en: "Recent Orders",
          es: "Pedidos recientes",
        },
        my_order: {
          en: "My Orders",
          es: "Mis pedidos",
        },

        update_profile: {
          en: "Update Profile",
          es: "Actualizar Perfil",
        },
        full_name: {
          en: "Full Name",
          es: "Nombre Completo",
        },
        address: {
          en: "Address",
          es: "Direccion",
        },
        user_phone: {
          en: "Phone/Mobile",
          es: "Telefono o celular",
        },
        user_email: {
          en: "Email Address",
          es: "Direccion de correo electronico",
        },
        update_button: {
          en: "Update Profile",
          es: "Actualizar Perfil",
        },
        current_password: {
          en: "Current Password",
          es: "Contraseña actual",
        },
        new_password: {
          en: "New Password",
          es: "Nueva contraseña",
        },
        change_password: {
          en: "Change Password",
          es: "Cambiar la contraseña",
        },
      },

      footer: {
        promo_status: true,
        block1_status: true,
        block2_status: true,
        block3_status: true,
        block4_status: true,
        payment_method_status: true,
        bottom_contact_status: true,
        social_links_status: true,
        shipping_card: {
          en: "Free Shipping From €500.00",
          es: "Envío gratuito a partir de 500,00€",
        },
        support_card: {
          en: "Support 24/7  At Anytime",
          es: "Soporte 24/7 en cualquier momento",
        },
        payment_card: {
          en: "Secure Payment  Totally Safe",
          es: "Pago seguro, totalmente confiable.",
        },
        offer_card: {
          en: "Latest Offer Upto 20% Off",
          es: "Última oferta: hasta un 20% de descuento",
        },
        block1_title: {
          en: "Company",
          es: "Compañía",
        },
        block1_sub_title1: {
          en: "About Us",
          es: "Sobre nosotros",
        },
        block1_sub_link1: "/about-us",
        block1_sub_title2: {
          en: "Contact Us",
          es: "Contacta con nosotros",
        },
        block1_sub_link2: "/contact-us",
        block1_sub_title3: {
          en: "Careers",
          es: "Carreras",
        },
        block1_sub_link3: "#",
        block1_sub_title4: {
          en: "Latest News",
          es: "Últimas noticias",
        },
        block1_sub_link4: "#",
        block2_title: {
          en: "Latest News",
          es: "Últimas noticias",
        },
        block2_sub_title1: {
          en: "Fish & Meat",
          es: "Pescado y Carne",
        },
        block2_sub_link1: "/search?category=fish-meat",
        block2_sub_title2: {
          en: "Soft Drink",
          es: "Refresco",
        },
        block2_sub_link2: "/search?category=drinks",
        block2_sub_title3: {
          en: "Milk & Dairy",
          es: "Leche y Lácteos",
        },
        block2_sub_link3: "/search?category=milk-dairy",
        block2_sub_title4: {
          en: "Beauty & Health",
          es: "Belleza y salud",
        },
        block2_sub_link4: "/search?category=beauty-health",
        block3_title: {
          en: "My Account",
          es: "Mi cuenta",
        },
        block3_sub_title1: {
          en: "Dashboard",
          es: "Panel de control",
        },
        block3_sub_link1: "/user/dashboard",
        block3_sub_title2: {
          en: "My Orders",
          es: "Mis pedidos",
        },
        block3_sub_link2: "/user/my-orders",
        block3_sub_title3: {
          en: "Recent Orders",
          es: "Pedidos recientes",
        },
        block3_sub_link3: "/user/dashboard",
        block3_sub_title4: {
          en: "Update Profile",
          es: "Actualizar perfil",
        },
        block3_sub_link4: "/user/update-profile",
        block4_logo:
          "https://res.cloudinary.com/ahossain/image/upload/v1697688576/settings/logo-color_el4zmy.svg",
        block4_address: {
          en: "987 Andre Plain Suite High Street 838, Lake Hestertown, USA",
          es: "987 Andre Plain Suite High Street 838, Lake Hestertown, EE. UU.",
        },
        block4_phone: "02.356.1666",
        block4_email: "ccruidk@test.com",
        social_facebook: "https://www.facebook.com/",
        social_twitter: "https://twitter.com/",
        social_pinterest: "https://www.pinterest.com/",
        social_linkedin: "https://www.linkedin.com/",
        social_whatsapp: "https://web.whatsapp.com/",
        payment_method_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1697688607/settings/payment-logo_qhslgz.webp",
        bottom_contact: "+6599887766",
      },
      slug: {
        right_box_status: true,
        card_description_one: {
          en: "Free shipping applies to all orders over shipping €100",
          es: "El envío gratuito aplica a todos los pedidos superiores a 100 €",
        },
        card_description_two: {
          en: "Home Delivery within 1 Hour",
          es: "Entrega a domicilio en 1 hora",
        },
        card_description_three: {
          en: "Cash on Delivery Available",
          es: "Pago contra entrega disponible",
        },
        card_description_four: {
          en: "7 Days returns money back guarantee",
          es: "7 días de garantía de devolución de dinero",
        },
        card_description_five: {
          en: "Warranty not available for this item",
          es: "Garantía no disponible para este artículo",
        },
        card_description_six: {
          en: "Guaranteed 100% organic from natural products.",
          es: "Garantizado 100% orgánico a partir de productos naturales.",
        },
        card_description_seven: {
          en: "Delivery from our pick point Boho One, Bridge Street West, Middlesbrough, North Yorkshire, TS2 1AE.",
          es: "Entrega desde nuestro punto de recogida Boho One, Bridge Street West, Middlesbrough, North Yorkshire, TS2 1AE.",
        },
      },
      seo: {
        favicon:
          "https://res.cloudinary.com/ahossain/image/upload/v1700903173/undefined/favicon.png",
        meta_description:
          "React Grocery & Organic Food Store e-commerce Template",
        meta_img:
          "https://res.cloudinary.com/ahossain/image/upload/v1636729752/facebook-page_j7alju.png",
        meta_keywords: "e-commerce, online store",
        meta_title:
          "KachaBazar - React Grocery & Organic Food Store e-commerce Template",
        meta_url: "https://kachabazar-store-nine.vercel.app/",
      },
    },
    name: "storeCustomizationSetting",
  },
  {
    setting: {
      cod_status: true,
      qr_status: false,
      fb_pixel_key: "",
      fb_pixel_status: false,
      google_analytic_key: "",
      google_analytic_status: false,
      google_login_status: false,
      github_login_status: false,
      facebook_login_status: false,
      google_id: "",
      google_secret: "",
      github_id: "",
      github_secret: "",
      facebook_id: "",
      facebook_secret: "",
      nextauth_secret: "",
      next_api_base_url: "http://localhost:5055/api",
      qr_key: "",
      qr_secret: "",
      stripe_key: "pk_test_key",
      stripe_secret: "sk_test_secret",
      stripe_status: true,
      razorpay_status: false,
      razorpay_id: "",
      razorpay_secret: "",
      meta_url: "https://kachabazar-store-nine.vercel.app/",
      tawk_chat_property_id: "",
      tawk_chat_status: false,
      tawk_chat_widget_id: "",
      available_locations: [
        { key: "e8bd0b42-1b82-4ef3-b1e5-1a83e5c9c71a", label: "La Paz" },
        { key: "63f1c42d-28c5-47c4-9a38-4dbee89c9b20", label: "Santa Cruz" },
        { key: "a3473a18-2b8f-4c0a-94d0-fc51e5c2a634", label: "Cochabamba" },
      ],
    },
    name: "storeSetting",
  },
  {
    setting: {
      points: [
        {
          id: 1,
          location: "a3473a18-2b8f-4c0a-94d0-fc51e5c2a634",
          placeName: "Correo",
          streetName: "Av. Ayacucho",
          details: "Gradas del correo",
          shippingCost: 10,
        },
        {
          id: 2,
          location: "e8bd0b42-1b82-4ef3-b1e5-1a83e5c9c71a",
          placeName: "Plaza colon",
          streetName: "Av. Ballivian",
          details: "Puente de la plaza",
          shippingCost: 20,
        },
      ],
    },
    name: "deliveryPoints",
  },
];

module.exports = baseSettings;