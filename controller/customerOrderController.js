...
const updateExistingPaymentIntent = async (stripeInstance, payment_intent, amount, res) => {
  try {
    const current_intent = await stripeInstance.paymentIntents.retrieve(payment_intent.id);
    if (!current_intent) return false;

    const updated_intent = await stripeInstance.paymentIntents.update(payment_intent.id, {
...
const createPaymentIntent = async (req, res) => {
  ...
  if (payment_intent?.id) {
    const updated = await updateExistingPaymentIntent(stripeInstance, payment_intent, amount, res);
    if (updated) return;
  }
